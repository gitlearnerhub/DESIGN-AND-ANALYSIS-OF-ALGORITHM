...
{ 
	int *maxRevenue = new int[tot_miles+1]; 			
...
int main() 
{  /*
        Objective       : Program to calculate maximum revenue by placing a subset of billboard on the highway
    ...
    */
	...
	while(true){
		...
		if(site[count - 1] < 0)
			break;
		//checking whether sites are inserted in increasing order
